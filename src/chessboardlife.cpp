...
void Agent::step()
{
	...
	// осматриваемс€, оцениваем обстановку, принимаем решени€...
	{
		...
		// здесь действи€, св€занные со зрением
...
void TurnLeftAction::moveFunction()
{
	...
	// мен€ем местами первый и второй компоненты, а затем
	// мен€ем знак второму компоненту, т.е.
	// (x, y) превращаетс€ в (y, -x)
...
void TurnRightAction::moveFunction()
{
	...
	// мен€ем местами первый и второй компоненты, а затем
	// мен€ем знак первому компоненту, т.е.
	// (x, y) превращаетс€ в (-y, x)
	spt->setOrientation({ -y, x });
}
...
void ChessboardLifeViewer::step()
{
	...
	if (_p->window->isOpen()) {
		...
		// вьюпорт - область, куда будем выводить, но заданна€ в дол€х от размера окна (!):
